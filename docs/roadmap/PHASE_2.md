# Phase 2: 个人照片库

> 版本: v1.2.0 | 状态: 📋 待开发 | 依赖: Phase 1

## 目标

允许登录用户保存照片到个人账户，提供照片库管理界面，支持分享到公共画廊。

## 核心功能

- [ ] 照片保存到云端
- [ ] 个人照片库界面
- [ ] 照片查看/删除
- [ ] 分享到公共画廊
- [ ] 照片下载

## 前置条件

- ✅ Phase 1 完成 (用户认证系统)
- ✅ 用户可以正常登录

## 任务清单

### 任务 2.1: 数据库表创建
**状态**: 📋 待开发

**目标**: 创建 user_photos 表，修改 public_photos 表

**SQL 脚本**:
```sql
-- 创建 user_photos 表
create table user_photos (
  id uuid default gen_random_uuid() primary key,
  user_id uuid references auth.users(id) on delete cascade not null,
  data_url text not null,
  caption text,
  frame_style text not null default 'classic',
  filter_id text,
  pokemon_id text,
  prompt_used text,
  is_public boolean default false,
  created_at timestamp with time zone default timezone('utc', now()) not null
);

-- 索引
create index idx_user_photos_user_id on user_photos(user_id);
create index idx_user_photos_created_at on user_photos(created_at desc);

-- RLS
alter table user_photos enable row level security;

create policy "Users can view own photos" on user_photos
  for select using (auth.uid() = user_id);
create policy "Users can insert own photos" on user_photos
  for insert with check (auth.uid() = user_id);
create policy "Users can update own photos" on user_photos
  for update using (auth.uid() = user_id);
create policy "Users can delete own photos" on user_photos
  for delete using (auth.uid() = user_id);

-- 修改 public_photos 表
alter table public_photos
  add column user_id uuid references auth.users(id) on delete set null,
  add column source_photo_id uuid references user_photos(id) on delete set null;

create index idx_public_photos_user_id on public_photos(user_id);
```

**验收标准**:
- [ ] user_photos 表创建成功
- [ ] public_photos 新字段添加
- [ ] RLS 策略生效

---

### 任务 2.2: photoService 服务层
**状态**: 📋 待开发

**目标**: 创建照片存储服务

**文件**: `src/services/photoService.ts`

**方法列表**:
```typescript
// 保存照片到个人库
savePhoto(photo: PhotoData, userId: string): Promise<SavedPhoto>

// 获取用户所有照片
getUserPhotos(userId: string): Promise<SavedPhoto[]>

// 删除照片
deletePhoto(photoId: string, userId: string): Promise<void>

// 分享到公共画廊
shareToPublic(photoId: string, userId: string): Promise<void>

// 取消公开
unshareFromPublic(photoId: string, userId: string): Promise<void>

// 更新照片标题
updatePhotoCaption(photoId: string, userId: string, caption: string): Promise<void>
```

**类型定义**:
```typescript
interface SavedPhoto {
  id: string;
  userId: string;
  dataUrl: string;
  caption?: string;
  frameStyle: string;
  filterId?: string;
  pokemonId?: string;
  promptUsed?: string;
  isPublic: boolean;
  createdAt: string;
}
```

**验收标准**:
- [ ] 所有方法实现
- [ ] 错误处理完善
- [ ] RLS 权限验证

---

### 任务 2.3: useMyPhotos Hook
**状态**: 📋 待开发

**目标**: 创建个人照片管理 Hook

**文件**: `src/hooks/useMyPhotos.ts`

**接口**:
```typescript
function useMyPhotos() {
  return {
    // 状态
    photos: SavedPhoto[];
    loading: boolean;
    error: Error | null;

    // 方法
    savePhoto: (photo: PhotoData) => Promise<SavedPhoto>;
    deletePhoto: (photoId: string) => Promise<void>;
    shareToPublic: (photoId: string) => Promise<void>;
    refresh: () => Promise<void>;
  };
}
```

**实现要点**:
- 依赖 useAuth 获取用户状态
- 未登录时返回空数组
- 自动加载用户照片
- 操作后自动更新列表

**验收标准**:
- [ ] Hook 正常工作
- [ ] 状态正确更新
- [ ] 加载状态处理

---

### 任务 2.4: MyGallery 组件
**状态**: 📋 待开发

**目标**: 创建个人照片库界面

**文件**: `src/components/gallery/MyGallery.tsx`

**Props**:
```typescript
interface MyGalleryProps {
  isOpen: boolean;
  onClose: () => void;
}
```

**UI 设计**:
```
┌─────────────────────────────────────────────────────────┐
│  ←  我的照片库                        筛选 ▼   🔍 搜索 │
├─────────────────────────────────────────────────────────┤
│                                                         │
│    📸 共 24 张照片                                      │
│                                                         │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐   │
│  │ 🌍      │  │         │  │         │  │         │   │
│  │  照片1  │  │  照片2  │  │  照片3  │  │  照片4  │   │
│  │         │  │         │  │         │  │         │   │
│  │ 2024/1/1│  │ 2024/1/2│  │ 2024/1/3│  │ 2024/1/4│   │
│  └─────────┘  └─────────┘  └─────────┘  └─────────┘   │
│                                                         │
│  ┌─────────┐  ┌─────────┐  ...                        │
│  │         │  │         │                              │
│  └─────────┘  └─────────┘                              │
│                                                         │
│  ───────── 加载更多 ─────────                          │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

**功能说明**:
- 网格展示 (2-4 列响应式)
- 已公开照片显示 🌍 图标
- 显示拍摄日期
- 点击照片显示详情
- 筛选: 全部/已公开/滤镜/相框
- 空状态提示

**样式要求**:
- 全屏覆盖层模式
- 头部固定 + 内容滚动
- 照片卡片带阴影和 hover 效果

**验收标准**:
- [ ] 正确显示用户照片
- [ ] 网格布局响应式
- [ ] 筛选功能正常
- [ ] 空状态处理

---

### 任务 2.5: PhotoActions 组件
**状态**: 📋 待开发

**目标**: 照片操作菜单组件

**文件**: `src/components/gallery/PhotoActions.tsx`

**Props**:
```typescript
interface PhotoActionsProps {
  photo: SavedPhoto;
  onDelete: () => void;
  onShare: () => void;
  onUnshare: () => void;
  onDownload: () => void;
  onClose: () => void;
}
```

**UI 设计**:
```
┌─────────────────────────────────────────┐
│                                         │
│          [照片预览]                     │
│                                         │
├─────────────────────────────────────────┤
│                                         │
│   标题: 周末的阳光 ✏️                   │
│   时间: 2024年1月15日                   │
│   滤镜: 1977                            │
│   相框: 经典白                          │
│                                         │
├─────────────────────────────────────────┤
│                                         │
│   ┌─────────┐  ┌─────────┐             │
│   │ 📥 下载 │  │ 🌍 分享 │             │
│   └─────────┘  └─────────┘             │
│                                         │
│   ┌─────────────────────┐              │
│   │    🗑️ 删除照片      │              │
│   └─────────────────────┘              │
│                                         │
└─────────────────────────────────────────┘
```

**功能说明**:
- 照片大图预览
- 显示照片元信息
- 编辑标题 (点击 ✏️)
- 下载原图 (PNG)
- 分享/取消分享到公共画廊
- 删除照片 (带确认)

**验收标准**:
- [ ] 照片预览正常
- [ ] 所有操作可用
- [ ] 删除有确认提示
- [ ] 操作后列表更新

---

### 任务 2.6: PolaroidPhoto 保存按钮
**状态**: 📋 待开发

**目标**: 在照片组件上添加保存按钮

**修改文件**: `src/components/PolaroidPhoto.tsx`

**添加内容**:
```tsx
// 保存按钮 (仅 DONE 状态显示)
{photo.status === PhotoStatus.DONE && (
  <button
    className="absolute top-2 left-2 w-8 h-8 bg-white/80 rounded-full
               flex items-center justify-center shadow-md
               hover:bg-white hover:scale-110 transition-all"
    onClick={handleSave}
    title={t.savePhoto}
  >
    💾
  </button>
)}
```

**交互逻辑**:
```
点击保存
    │
    ▼
┌─────────┐
│ 已登录? │
└────┬────┘
     │
  ┌──┴──┐
  No    Yes
  │     │
  ▼     ▼
弹出   保存成功
登录   显示 ✓
```

**验收标准**:
- [ ] 按钮正确显示
- [ ] 未登录弹出登录框
- [ ] 保存成功有反馈
- [ ] 按钮不影响拖拽

---

### 任务 2.7: UserMenu 集成
**状态**: 📋 待开发

**目标**: 在用户菜单添加"我的照片库"入口

**修改文件**: `src/components/auth/UserMenu.tsx`

**添加内容**:
```tsx
// 菜单项
<button onClick={() => setShowGallery(true)}>
  📸 我的照片库 ({photoCount})
</button>

// MyGallery 组件
<MyGallery
  isOpen={showGallery}
  onClose={() => setShowGallery(false)}
/>
```

**显示照片数量**:
- 使用 useMyPhotos 获取照片列表
- 显示总数: `我的照片库 (24)`

**验收标准**:
- [ ] 菜单项可点击
- [ ] 显示照片数量
- [ ] 打开/关闭画廊正常

---

## 文件创建清单

| 文件路径 | 任务 | 说明 |
|----------|------|------|
| `src/services/photoService.ts` | 2.2 | 照片服务 |
| `src/hooks/useMyPhotos.ts` | 2.3 | 照片 Hook |
| `src/components/gallery/MyGallery.tsx` | 2.4 | 个人画廊 |
| `src/components/gallery/PhotoActions.tsx` | 2.5 | 照片操作 |

## 修改文件清单

| 文件路径 | 任务 | 修改内容 |
|----------|------|----------|
| `src/components/PolaroidPhoto.tsx` | 2.6 | 添加保存按钮 |
| `src/components/auth/UserMenu.tsx` | 2.7 | 添加画廊入口 |
| `src/constants.ts` | 2.4-2.6 | 添加翻译文本 |

## 用户流程

### 保存照片
```
1. 用户拍照完成 (DONE 状态)
2. 照片左上角显示 💾 保存按钮
3. 用户点击保存
4. [未登录] 弹出登录框 → 登录成功后继续
5. [已登录] 调用 photoService.savePhoto()
6. 显示成功提示 (按钮变 ✓)
7. 照片存入 user_photos 表
```

### 查看照片库
```
1. 用户点击头像 → 我的照片库
2. 打开 MyGallery 全屏界面
3. 加载用户照片列表
4. 网格展示所有照片
5. 用户可筛选/搜索
```

### 管理照片
```
1. 在照片库中点击某张照片
2. 打开 PhotoActions 弹窗
3. 查看照片详情
4. 可选操作:
   - 下载原图
   - 分享到公共画廊
   - 编辑标题
   - 删除照片
```

### 分享到公共画廊
```
1. 在 PhotoActions 中点击"分享"
2. 确认分享操作
3. 复制照片到 public_photos
4. 照片标记为 is_public = true
5. 照片库中显示 🌍 图标
6. 可在公共画廊中看到
```

## 测试用例

### 功能测试
- [ ] 保存照片成功
- [ ] 保存重复照片处理
- [ ] 照片库加载正常
- [ ] 分页/无限滚动
- [ ] 筛选功能正确
- [ ] 删除后列表更新
- [ ] 分享/取消分享

### 边界测试
- [ ] 无照片时空状态
- [ ] 大量照片性能
- [ ] 网络错误处理
- [ ] 并发操作处理

## 注意事项

1. **图片大小**
   - Base64 图片较大，注意加载性能
   - 可考虑生成缩略图

2. **RLS 安全**
   - 确保用户只能访问自己的照片
   - 删除操作需验证所有权

3. **分享逻辑**
   - 分享是复制，不是移动
   - 取消分享只删除公共画廊副本

4. **状态同步**
   - 保存后本地状态无需更新 (照片仍在画布)
   - 删除后需从画廊列表移除

## 完成标准

Phase 2 完成的标志：
- [ ] 登录用户可以保存照片
- [ ] 可以查看自己的照片库
- [ ] 可以删除照片
- [ ] 可以分享到公共画廊
- [ ] 可以下载原图
